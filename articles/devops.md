...
こちらが AWS における**ぼくのかんがえたさいきょうの DevOps 実現構成**の全体図です。前回より広い領域をカバーし、無駄な部分は省きました。簡単に説明すると「**GitHub + α**」⇄「**Grafana Cloud + α**」で(Dev⇄Ops)のループを回す構成になっています。

[![](https://images.microcms-assets.io/assets/0402567430be43b4a7db9f7365e62718/ea21ae688d1a4582bfdbd3bf55263bc5/devops.drawio.png)](https://images.microcms-assets.io/assets/0402567430be43b4a7db9f7365e62718/ea21ae688d1a4582bfdbd3bf55263bc5/devops.drawio.png)
...
![](https://storage.googleapis.com/zenn-user-upload/27947e906bed-20240229.png)
...
AWS のメトリクスとログの取得方法に関しては下記の公式ドキュメントを参照してください。
...
セキュリティの脆弱性を検出し、アプリケーションの安全性を確保することです。**Snyk**というツールを使用します。
...
**Qase**を使ってテストケースの管理をします。

<!-- textlint-disable spellcheck-tech-word -->

単体テストや結合テスト、E2E テストを自動化させましたが、全てのテストを自動化できるわけではなく、手動で行うべきテストもあります。
...
- **GitHub Actions**
  ...
  - GitHub Actions のログを Grafana Loki に送信するアクションを作ってみた（仮）

- **Grafana Cloud**

  - [負荷テストツール「k6」入門](https://zenn.dev/pharmax/articles/98ed49994cdaf2)
  - Grafana のダッシュボードを Slack に通知する Lambda を Terraform と Node.js で構築（仮）
  - アラートの管理に便利な「Grafana OnCall」を使ってみよう（仮）
...
- **Checkly**
  ...
  - 外形監視と E2E テストができる Checkly とは？（仮）
...
こちらはあくまでも 2024 年 3 月時点での構成なので、今後もどんどん更新していく(もしくは新しい記事を作成する)予定です。また、個々のサービスを掘り下げた記事も書いていこうと思うので、そちらもお読みいただければ幸いです！
