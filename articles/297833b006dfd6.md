...
こちらが**ぼくのかんがえたさいきょうの DevOps 実現構成**の全体図です。前回より広い領域をカバーし、無駄な部分は省きました。簡単に説明すると「**GitHub + α**」⇄「**Grafana Cloud + α**」で(Dev⇄Ops)のループを回す構成になっています。
...
[DevOps Research and Assessment（DORA）](https://dora.dev/)チームは、6 年間の研究を基に、ソフトウェア開発チームのパフォーマンスを示す 4 つの指標として下記の 4 つを挙げています。
...
また、「[Effective DevOps](https://www.oreilly.co.jp/books/9784873118352/)」には DevOps のアンチパターンとして下記の 4 つが紹介されています。
...
今回はツールの紹介となりますが、ツールの導入で満足するのではなく、ツールを通じて DevOps の**文化**を加速させることが重要となります。
...
DevOps の**文化**を加速させるためにも、ChatOps は非常に重要です。

# DevOps の文化を加速させる

<!-- textlint-disable ja-technical-writing/ja-no-mixed-period -->

「[DevOps とは](#devops-とは)」において、

<!-- textlint-enable -->

- 「[CALMS](#devops-と「calms」)」の１つは**Culture**(**文化**)
- 「[3 つの道](#devops-の「3-つの道」)」の**継続的な学習と実験の原則**は文化に関するもの
- 「[4 本柱](#devops-の「4-本柱」)」のツール以外の全てが**文化**に関するもの
  - コラボレーション
  - アフィニティ
  - スケーリング
- 「[ケイパビリティ](#devops-の能力（ケイパビリティ）)」の 3 つの分類のうちの１つが**文化に関するケイパビリティ**
  - 創造的な組織文化
  - 仕事の満足度
  - 学習文化
  - 変革型リーダーシップ
...
また「[DevOps は”文化”](#devops-は”文化”)」でも紹介したように、ツールは**文化**を加速させるためのものでしかない旨が書かれていたりしました。

このように DevOps は**文化**が非常に重要であることが分かります。

そして、「[Effective DevOps](https://www.oreilly.co.jp/books/9784873118352/)」では**ツールの誤解**として、下記を挙げていました。

- 「**技術*X*から、他社に合わせて技術 Y に移行しなければいけない**」
  文化には技術スタックが含まれており、それを全体的に変更すると組織全体の足を引っ張るようなコストがかかる。
- 「**技術*X*を使っているので、うちは DevOps を実践している**」
  DevOps に役立つツールや技術は確かにあるが、それらのツールや技術がなぜ役に立つのかを理解するのが大切。
- 「**間違ったツールを選ばないように注意しなければならない**」
  間違ったツールを選んだからといって失敗する訳ではない。
- 「**DevOps ツール全部入りセットや DevOps-as-a-Service を購入すればよい**」
  最新の素晴らしい DevOps のツールを持っているだけでは不十分で、成功するにはツールを効果的に使わなければならない。

これらから、ツールを導入することが DevOps ではないということが分かります。

本記事では様々なツールを紹介しましたが、これらのツールが正解という訳ではありませんし、これらを導入することが DevOps という訳ではありません。

とはいえ、ツールは**文化**を加速させるために必要なものということは間違いありません。本記事を参考にしつつ現在の組織に合った適切なツールを選択していただければ幸いです。
...
何度もお伝えしますが、DevOps はツールだけでは完成しません。ツールの導入で満足するのではなく、ツールを通じて DevOps の文化を加速させていきましょう。
