...
本記事では**ぼくのかんがえたさいきょうの運用・監視構成**を進化させた**ぼくのかんがえたさいきょうの DevOps 実現構成**紹介させていただきます。あくまでも「ぼくのかんがえた」なので私個人の意見として受け入れていただけると助かります 🙇‍♂️

前回の記事でもお伝えいたしましたが、各個人・企業によって環境は違うと思いますし、使いやすいサービスは人それぞれだと思います。これが正解という訳ではありませんが、参考にしてただければ幸いです。また、こちらの記事は 2024 年 3 月時点での情報です。料金改定により、他のツールのコストパフォーマンスが向上したり、新たに便利なサービスが登場する可能性もあります。

# 全体図

こちらが AWS における**ぼくのかんがえたさいきょうの DevOps 実現構成**の構成図です。前回より広い領域をカバーし、無駄な部分は省きました。簡単に説明すると「**GitHub・GitHub Actions**」⇄「**Grafana Cloud + α**」で(Dev⇄Ops)のループを回す構成になっています。
...
これだけでは DevOps に具体的にイメージしづらい思うので、DevOps の指標をいくつか紹介します。
それぞれ共通する部分が多いのでイメージしやすいのではないでしょうか。
...
（多いので詳細については省きます。先ほど紹介したサイトを参照してみてください）
...
との記載があるのでこちらの書籍で紹介されているアンチパターンを DevOps のアンチパターンとして紹介します。
...
ちなみにトレースとは、メトリクスやログなどと同じくアプリケーションを監視するためのデータの 1 つで、メトリクス・ログ・トレースを３つ合わせてオブザーバビリティの 3 本柱と言われており、複数のサーバーを伝播する特定のリクエストの追跡データを取得します。
...
ちなみに継続的プロファイリングとは先ほどのオブザーバビリティの 3 本柱に次ぐ 4 本目の柱とも言われており、アプリケーションのどのコードがどの程度の CPU やメモリを消費しているか、実行時間がどのくらいかかっているかを把握するためのデータを取得します。
...
ちなみにエラートラッキングとはアプリケーションのエラーを監視することです。ログとの違いについては[こちら](https://sentry.io/vs/logging/)をご覧ください。
...
ちなみにフィーチャーフラグは、コードを直接変更せずに、特定の機能をオンまたはオフにできる技術です。これにより、新機能を特定のユーザーのみに表示したり、AB テストなどの機能テストを行うことが可能です。
...
JavaScript だとフォーマットには[**Prettier**](https://prettier.io/)、静的解析には[**ESlint**](https://eslint.org/)が有名で、新しいツールだとフォーマットと静的解析の両方が行えて、Prettier と ESLint との高い互換性のある[**Biome**](https://biomejs.dev/ja/)といったツールも注目を浴びています。また、Go だと標準でフォーマットや静的解析の機能が用意されていたりします。

今回の構成では**Biome**を記載していますが、言語やツールの好みによって変わってくるかと思います。

## 単体テスト&結合テスト(Vitest, etc.)

単体テストは関数やクラスが正しく機能するかをチェックするテストのことです。
JavaScript だと[**Jest**](https://jestjs.io/ja/)が有名で、最近は Jest と高い互換性があり Jest よりも高速な[**Vitest**](https://vitest.dev/)といったツールも注目を浴びています。また、Go だと標準でテストの機能が用意されていたりします。
...
今回の構成ではテストのツール例として**Vitest**を記載していますが、言語やツールの好みよって変わってくるかと思います。

## ビルド(Vite, etc.)

ソースコードから実行可能なプログラムを生成するプロセスのことです。
JavaScript だと[**webpack**](https://webpack.js.org/)というバンドルツールが有名ですが、新しいツールだと[**Vite**](https://ja.vitejs.dev/)や webpack の後継である[**Turbopack**](https://turbo.build/pack)が webpack よりも高速で注目を浴びています。また、Go だと標準でビルドの機能が用意されていたりします。

今回の構成では**Vite**を記載していますが、言語やツールの好みよって変わってくるかと思います。
...
ソフトウェアが全体的に、エンドユーザーの視点で正しく機能するかを確認するテストのことです。[**Playwright**](https://playwright.dev/)といったツールが有名で、**Checkly**というツールにはこの**Playwright**が組み込まれているので、この**Checkly**を使用します。

今回の構成では GitHub Actions 上で Checkly CLI を動かして、**Checkly**という SaaS で E2E テストを実行します。[**Checkly**については後述します。](<#外形監視とe2eテスト(checkly)>)

## 負荷テスト(Grafana k6)

システムに負荷をかけて、パフォーマンスの影響や不具合を確認するテストのことです。**Grafana k6**というツールを使用します。
...
PR のレビューはレビュー側にも知識が必要ですし、規模の大きい PR はレビューをするのも大変です。ですので、**CodeRabbit**を使って PR のレビューを効率化させます。**CodeRabbit**は PR の要約と AI によるレビューをしてくれる SaaS です。
...
**Grafana Cloud**を使用して監視とオブザーバビリティを向上します。**Grafana Cloud**は後述する[**Grafana**](#ダッシュボード（grafana）)、[**Prometheus**](<#メトリクス(prometheus)>)、[**Grafana Loki**](<#ログ(grafana-loki)>)、[**Grafana Tempo**](<#トレース(grafana-tempo)>)、[**Grafana Pyroscope**](<#継続的プロファイリング(grafana-pyroscope)>)を Grafana Labs がマネージドサービスとして提供している SaaS です。
