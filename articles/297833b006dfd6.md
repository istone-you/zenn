...
※`$ snyk test`でローカルの実行、`$ snyk monitor`で Web 上の Snyk にプロジェクトを作成して、継続的な脆弱性スキャンを行うことができます。
...
GitHub Actions 上で Playwight CLI もしくは Checkly CLI を動かして E2E テストを CI に組み込んだり、Web 上の**Checkly**で定期的な E2E テストを行う事もできます。

![](https://storage.googleapis.com/zenn-user-upload/4ca482d3189f-20240326.png =500x)

※`$ checkly test`でローカルの実行、`$ checkly deploy`で Web 上の Checkly にデプロイして、継続的な E2E テストを行うことができます。
