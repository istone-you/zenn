...
今回はツールの紹介となりますが、ツールの導入で満足するのではなく、ツールを通じて DevOps の**文化**を加速させることが大切です。
...
Grafana には GitHub をデータソースとして利用できるプラグインが用意されており、GitHub のデータを可視化すできます。

「Four Keys」の 4 つの指標のうち、**デプロイの頻度**や**変更のリードタイム**を把握するためにも有用です。
...
「Four Keys」の 4 つの指標のうち、**サービスの復旧時間**を把握するためにも有用です。
...
**Grafana Cloud**を使用して監視とオブザーバビリティを向上します。
...
「Four Keys」の 4 つの指標のうち、**サービスの復旧時間**を把握するために有用です。
...
![](https://storage.googleapis.com/zenn-user-upload/242e6162a1ba-20240302.png)

性質テストはパフォーマンスやセキュリティ等のテストを指します。こちらはすでに紹介した [**k6**](<#負荷テスト(grafana-k6)>)やこの[**Snyk**](<#セキュリティスキャン(snyk)>) でカバーしています。
